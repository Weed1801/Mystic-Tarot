...
const QuestionInput = ({ onStart }) => {
    ...
    return (
        <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="w-full max-w-md mx-auto z-50 px-4"
        >
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 bg-black/40 backdrop-blur-md p-6 md:p-8 rounded-2xl border border-white/10 shadow-2xl">
                <h2 className="text-xl md:text-2xl font-serif text-mystic-gold text-center mb-2">Mystic Tarot</h2>
                <p className="text-gray-300 text-center text-sm mb-4">Hãy tập trung vào câu hỏi của bạn, và để những lá bài dẫn lối.</p>

                <input
                    type="text"
                    value={question}
                    onChange={(e) => setQuestion(e.target.value)}
                    placeholder="Tương lai của tôi sẽ như thế nào?"
